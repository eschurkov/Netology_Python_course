...
def read_file(file):
	with codecs.open(file['name'], encoding=file['encoding']) as news_file:
		#print (json.load(news_file))
		return json.load(news_file)


def count_words(news):
	words = Counter()
	# пока не знаю, как сделать так, чтобы и сслки не попадали  перечень слов и все слэши '/' убрать
	clean_trash = re.compile('\W+ ?')
	for k in range(len(news['rss']['channel']['item'])):
		words += Counter(
			# чистим строку от лишних знкаков, <br>
			clean_trash.sub(' ', news['rss']['channel']['item'][k]['description']['__cdata'].replace('<br>','')).split()
			)
...
def print_words(filename, words):
	print('\nПервая десятка слов больше 6 символов в текстах новостей файла', file['name'])
	for word in words:
